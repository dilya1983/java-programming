package day30_arrays;

public class StudentArray {
    public static void main(String[] args) {

        String [] student1 = new String[5];
         student1[0] = "AD1234";
         student1[1] = "Adam";
         student1[2] = "Smith";
         student1[3] = "B22";
         student1[4] = "202-503-1234";

        String [] student2 = {"1234","Adam","khamidov","B22","347-946-4444"};

        System.out.println("student id = " + student1[0]);
        System.out.println("student firstname = " + student1[1]);
        System.out.println("student lastName = " + student1[2]);
        System.out.println("student batch num = " + student1[3]);
        System.out.println("student mobil num = " + student1[4]);

        System.out.println("student data length: " + student1.length);


        if(student1.length == 5){
            System.out.println(" Pass: data array has correct length");
        }else{
            System.out.println(" Fail: data array has  incorrect length");
        }
        if(student1.length == student2.length){
           System.out.println(" PASS: data array has correct length");
        }else{
           System.out.println(" FAIL: data array has  incorrect length");

       }
        System.out.println("student1[1].toUpperCase() = " + student1[1].toUpperCase());
        System.out.println("student1[2].toUpperCase() = " + student1[2].toUpperCase());
        System.out.println(student1[1].toUpperCase() + " " + student1[2].toUpperCase());

        String mobileNum = student1[4];
        System.out.println(mobileNum.startsWith("202"));


    }
}
